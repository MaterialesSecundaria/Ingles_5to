...
var projectName="Full Blast Plus B1+ Greek";
...
//Readers Spread Title
	...
	//previous
	if(current==0||Frame[current].Salon!=Frame[current-1].Salon||Frame[current].Extra==true){
		if(current==0||Frame[current].Salon!=Frame[current-1].Salon){$("[data-frame=previous]").attr("data-disabled","true");console.log("a1");}
		else{
			if(Frame[current].Extra!=Frame[current-1].Extra&&Frame[current].Salon==Frame[current-1].Salon){
				$("[data-frame=previous]").attr("data-disabled","true");console.log("b1");
			}
			else{
				$("[data-frame=previous]").removeAttr("data-disabled");console.log("c1");
			}
		}
	}
	else{
		if(Frame[current].Extra==Frame[current-1].Extra){
			$("[data-frame=previous]").removeAttr("data-disabled");console.log("d1");
		}
		else{
			$("[data-frame=previous]").attr("data-disabled","true");console.log("e1");
		}
	}
	//next
	if(current==Frame.length-1||Frame[current].Salon!=Frame[current+1].Salon||Frame[current+1].Extra==true){
		if(current==Frame.length-1){
			$("[data-frame=next]").attr("data-disabled","true");console.log("a2");
		}
		else{
			if(Frame[current].Extra==true&&Frame[current+1].Extra==true&&Frame[current].Salon==Frame[current+1].Salon){
				$("[data-frame=next]").removeAttr("data-disabled");console.log("b2");
			}
			else{
				$("[data-frame=next]").attr("data-disabled","true");console.log("c2");
			}
		}
	}
	else{
		if(Frame[current].Extra==Frame[current+1].Extra){
			$("[data-frame=next]").removeAttr("data-disabled");console.log("d2");
		}
		else{
			$("[data-frame=next]").attr("data-disabled","true");console.log("e2");
		}
	}
	...
	img.onload = function() {
		...
		if(Frame[current].Salon == true){
			...
			$.each(Frame[current].Lessons, function( index, value ) {
				...
				//Units
				var Unit = Frame[current].Unit;
				$("body").attr('data-unit',Unit);
			...
			//References
			if(Frame[current].Reference){
				$.each(Frame[current].Reference, function( index, value ){
					$(".reference").eq(value.id-1).append("<div data-sticker=\""+ value.Target +"\" data-type=\"RE\"><span>"+ value.Name +"</span></div>");
				});
				// $(".mainFrames").addClass("mainFramesNot");
			...
			try{
				if(Frame[current].Portal.length==1){
					...
					$.each(Frame[current].Portal, function( index, value ) {
						//Portal to grammar - has no video
					if(Frame[current].Portal[0].Video){
						var iconSvg=' <svg width="50px" height="50px" viewBox="0 0 50 50"><circle opacity="0.6" fill="#1E1E1E" cx="25" cy="25" r="25"/><path fill="#00FFFF" d="M38.4,21.6l-5.2,4.1v-2.4c0-1.7-1.3-3-3-3H16.1c-1.7,0-3,1.3-3,3v9.7c0,1.7,1.3,3,3,3h14.1c1.7,0,3-1.3,3-3			v-2.4l5.2,4.1c0.8,0.6,1.8-0.2,1.8-1.4V23.1C40.3,21.8,39.3,21,38.4,21.6z"/><path fill="#00FFFF" d="M18.1,18.9c2.4,0,4.3-1.9,4.3-4.3s-1.9-4.3-4.3-4.3c-2.4,0-4.3,1.9-4.3,4.3S15.8,18.9,18.1,18.9z"/><path fill="#00FFFF" d="M27.6,18.9c2.4,0,4.3-1.9,4.3-4.3s-1.9-4.3-4.3-4.3c-2.4,0-4.3,1.9-4.3,4.3S25.3,18.9,27.6,18.9z"/></svg> ';
					...
					if(Infos.Type!="Video Worksheets" && Infos.Type!="Worksheet"){
					...
					if(Infos.Type!="Video Worksheets"){
						var iconSvg=' <svg width="50px" height="50px" viewBox="0 0 50 50"><circle opacity="0.6" fill="#1E1E1E" cx="25" cy="25" r="25"/><path fill="#00FFFF" d="M38.4,21.6l-5.2,4.1v-2.4c0-1.7-1.3-3-3-3H16.1c-1.7,0-3,1.3-3,3v9.7c0,1.7,1.3,3,3,3h14.1c1.7,0,3-1.3,3-3			v-2.4l5.2,4.1c0.8,0.6,1.8-0.2,1.8-1.4V23.1C40.3,21.8,39.3,21,38.4,21.6z"/><path fill="#00FFFF" d="M18.1,18.9c2.4,0,4.3-1.9,4.3-4.3s-1.9-4.3-4.3-4.3c-2.4,0-4.3,1.9-4.3,4.3S15.8,18.9,18.1,18.9z"/><path fill="#00FFFF" d="M27.6,18.9c2.4,0,4.3-1.9,4.3-4.3s-1.9-4.3-4.3-4.3c-2.4,0-4.3,1.9-4.3,4.3S25.3,18.9,27.6,18.9z"/></svg> ';
			...
			if(Frame[current].SkipCorrelation){
				...
				$.each(Frame[current].SkipCorrelation, function( index, value ) {
					$.each(value.Correlation, function( index2, value2 ) {
						...
						if (value2=="none"){
							//console.log(value2);
							$(".correlation [data-type='" + value.Type + "']").eq(index2).fadeOut(0);
							}
					});
				});
			}
	...
	img.src = "../Images/"+ url +"/"+ Frame[current].Image +".jpg";
		...
		if(Frame[current].Salon == true){
			...
			if(Infos.Type!="Video Worksheets"){
				...
				$(".mainSvgs .mType").html("Video Worksheet");
			...
			if (Frame[current].Lesson.indexOf("pp.") != -1){$(".mainSvgs .mModule").html(Infos.ModuleText+" "+Frame[current].Lesson);}
		}
...
$(document).on("click", "[data-videotarget]", function () {
	...
	if($(this).attr("data-videotarget")=="video"){
		...
		$.each(Frame[currentFrame].Portal, function( index, value ) {
			...
			$(".videoExtra").append("<div "+ str +" data-targetsubs=\""+ value.id +"\"><span>"+ value.Worksheet + " " + Infos.Module +"</span></div>");
		});
	}
		...
		$.each(Frame[currentFrame].Portal, function( index, value ) {
			...
			$(".videoExtra").append("<div "+ str +" ><span>"+ value.Worksheet + " " + Infos.Module +"</span></div>");
		});
	}
	$(".videoWrapper").css("display","block");
});
...
$('.videoWrapper video').on('canplay', function(){
	...
	if(Frame[currentFrame].Parts){
		$.each(Frame[currentFrame].Parts, function( index, value ) {
			...
			$(".videoBar").append("<div data-videopart="+ value.time +" style='left:"+ left +"'>"+ (index+1) +"</div>");
						...
						if(value.TargetSub==value2.id){
							...
							if(value2.Parts){
								...
								$.each(value2.Parts, function( index3, value3 ) {
									var left4=value3.time/1000/$(".videoWrapper video")[0].duration*265+"px";
									$(".videoBar").append("<div data-videopart="+ value3.time +" style='left:"+ left4 +"'>"+ (index3+1) +"</div>");
								});
							}
...
$(document).on("click", "[data-type='EX']", function () {
	...
	var url = $(this).attr("data-type");
	window.open("../EX/Module"+ Infos.Module +".html",'_self');
...
$(document).on("click", ".mainBack", function () {
	...
	if(Infos.Type=="Grammar Tests" ){
		type="GT";
	}

		if(Infos.Type=="Module Tests"){
		type="TS";
	}
	...
	var Main={Module:Infos.Module,Type:type,Unit:Unit};
	window.name = '{"Module":"'+Infos.Module+'","Type":"'+type+'","Unit":"'+Unit+'"}';
	...
	if(!inp.hasClass("justActive")){
		...
		if(!$(this).attr("data-justext") && !$(this).attr("data-keyImg")){ //an den exei justification key, MONO Justifications
			...
			if(!$(this).attr("data-keyImg")){ //an exei justification keimeno kai den exei justification othoni (key image)
				...
				if($(".keyMask").height()+80>740){var imgTxtHeight = $(".keyMask").height()+80;}
